function Path = path_by_dijkstraCellPar(riddle,figureData)
...
TR = zeros(length(directions),length(startCell));
TRV = zeros(length(directions),length(start));
TC = cell(length(directions));
...
while(~sum(ismember(RV(:,1:3),target(1:3),'rows')))
    ...
    parfor i=1:length(directions) % für jede Richtung auf x,y
        ...
        TRV(i,:)=possible_next;
        TR(i,:)=newCell;
        TC{i} = next_collision_set;
        %wenn knoten nicht erlaubt ist nicht betrachten
    end
    
    for i=1:length(directions)
        newCell = TR(i,:);
        next_collision_set = TC{i};
        possible_next = TRV(i,:);
