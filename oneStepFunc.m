function [ nextNode, collision_set ] = oneStepFunc( node,direction,riddle,jump_over )
...
if mod(abs(direction),3) == 0
    ...
    riddle.o{object_pos} = rotateFunc(nextNode((object_pos-1)*3+1:object_pos*3),riddle.o{object_pos});
    collision_set = riddle.o;
    return;
end
...
for object_number=1:length(riddle.o) %iterate over all objects
    ...
    for function_number=1:length(object.coeff)
        ...
        %calculate min_y, max_y for linear function
        ...
        %get nextNode in direction direction. if function not in the way, dist = inf.
        [tempNode,dist] = moveToFunction(node,direction,riddle.o{object_pos},func,def);
        
        %save new minimum ( closest function in the way ) and new point on line
        if abs(dist) < min_dist && mind_dist>0
...
riddle.o{object_pos}=moveFunc(min_dist,direction,riddle.o{object_pos});
collision_set = riddle.o;
