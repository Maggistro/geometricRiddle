...
for func_number=1:length(object.coeff)
    ...
    if(object.coeff(1)==0)
        ...
        %rotate points
        rot_min=rotatePointAtPoint(min_point,mid,angle);      
        rot_max = rotatePointAtPoint(max_point,mid,angle); 
         %check if min is now max
         if(rot_min(1)>rot_max(1)) %change order
            %set new range borders
            object.def(1)=rot_max(1);
            object.def(2)=rot_min(1);
         
            %recalculate coefficients
            func(2) = (rot_min(2)-rot_max(2))/(object.def(2)-object.def(1));
            func(1) = rot_max(2) - func(2)*object.def(1);
         elseif(rot_min(1)==rot_max(1)) % REALLY BAD
            %TODO: think of someway to represent this
         else
            %set new range borders
            object.def(1)=rot_min(1);
            object.def(2)=rot_max(1);
         
            %recalculate coefficients
            func(2) = (rot_max(2)-rot_min(2))/(object.def(2)-object.def(1));
            func(1) = rot_min(2) - func(2)*object.def(1);
         end
